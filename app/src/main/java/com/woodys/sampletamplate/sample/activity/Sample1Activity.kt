...
import com.woodys.sampletamplate.sample.pojo.AccountInfo
...
class Sample1Activity : ToolBarActivity() {
    ...
    fun toPrinterActivityWithRequestLogin() {
        Router.create("woodys://page/intent/printer?title=动态登录检查&requestLogin=1").open(this)
    }

    
    fun toPrinterActivityWithInterceptor() {
        Router.create("woodys://page/intent/printer?title=使用指定拦截器")
    ...
    fun toPrinterActivityWithoutInterceptor() {
        Router.create("woodys://page/intent/printer?title=不使用拦截器进行跳转").open(this)
    }

   
    fun toUserActivity() {
        Router.create("woodys://page/user-info?username=测试账号").open(this)
    }
    ...
    fun toPrinterActivityWithExtras() {
        ...
        Router.create("woodys://page/intent/printer")
    ...
    fun toResultActivity() {
        Router.create("woodys://page/result")
